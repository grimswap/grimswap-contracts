...
/// @title SetupETHPool
/// @notice Initialize ETH/USDC pool with GrimSwapZK V3 hook + add liquidity
contract SetupETHPool is Script {
    ...
    // price = 2000 * 10^6 / 10^18 = 2e-9
    // sqrtPrice = sqrt(2e-9) * 2^96
    ...
    function run() external {
        ...
        console.log("=== Setup ETH/USDC Pool with V3 Hook ===");
        ...
        // 3. Add liquidity: 0.01 ETH + 20 USDC
        uint256 ethAmount = 0.01 ether;
        uint256 usdcAmount = 20 * 10**6; // 20 USDC (6 decimals)
        int24 tickLower = -276000; // Very wide range for test
        int24 tickUpper = -180000;

        IERC20(USDC).approve(address(helper), usdcAmount);
        console.log("  USDC approved");

        console.log("[3/3] Adding liquidity (0.01 ETH + 20 USDC)...");
        ...
        console.log("Pool: ETH/USDC with GrimSwapZK V3 hook");
